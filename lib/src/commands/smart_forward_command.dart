...
///
/// Reference: MS-ASCMD section 2.2.1.18
library;
...
class SmartForwardCommand {
  ...
  /// ServerId of the email being forwarded.
  final String serverId;

  /// CollectionId (folder) of the email being forwarded.
  final String collectionId;

  /// MIME content of the forward message (headers + body).
  final String mimeContent;

  /// Whether to save the forwarded message in Sent Items.
  ...
  Future<void> execute(EasHttpClient client) async {
    ...
    final response = await client.sendCommand('SmartForward', bytes);

    if (response.statusCode != 200) {
      throw EasCommandException(
        command: 'SmartForward',
        statusCode: response.statusCode,
        message: 'SmartForward failed with HTTP ${response.statusCode}',
      );
    }
    ...
    if (statusCode != 1) {
      throw EasCommandException(
        ...
        message: 'SmartForward error (status $statusCode)',
      );
    }
  }
}
